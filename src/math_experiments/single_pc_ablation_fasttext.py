...
FASTTEXT_RELATIVE = "data/wiki-news-300d-1M.vec"
...
DEFAULT_OUTPUT_CSV = "outputs/single_pc_ablation/fasttext/single_pc_ablation_results.csv"
DEFAULT_OUTPUT_PNG = "outputs/single_pc_ablation/fasttext/single_pc_ablation_plot.png"

DEFAULT_NEIGHBOR_K = 10
DEFAULT_NEIGHBOR_SAMPLE_SIZE = 2000
...
def get_repo_root() -> Path:
    # This assumes script is in src/math_experiments/
...
def load_fasttext_vec(path: str | Path) -> tuple[list[str], np.ndarray]:
    ...
    X = np.vstack([model[w] for w in vocab]).astype(np.float64)

    print(f"Loaded {len(vocab):,} FastText vectors with dimension {X.shape[1]}")
    return vocab, X
...
def compute_gender_pcs_from_definitional_pairs(
    ...
    if not diffs:
        raise ValueError("No definitional pairs found in FastText vocabulary.")
...
def save_plots(df: pd.DataFrame, out_png: str | Path) -> None:
    ...
    axes[0].set_title("FastText Single-PC Ablation: Bias Metrics")
    axes[0].legend()
    ...
    axes[1].set_title("FastText Single-PC Ablation: Geometry Metrics")
    axes[1].legend()
...
def run_experiment(args):
    vocab, X0 = load_fasttext_vec(args.embedding_path)
    ...
    print("\nBaseline metrics on original FastText embedding")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run FastText single-PC ablation experiment.")

    parser.add_argument(
        "--embedding-path",
        type=str,
        default=FASTTEXT_RELATIVE,
